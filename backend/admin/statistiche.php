...
$libriMonth = qall($conn, "
  SELECT DATE_FORMAT(COALESCE(data_pubblicazione, aggiunto_il),'%Y-%m') AS ym, COUNT(*) AS n
  FROM libri
  WHERE COALESCE(data_pubblicazione, aggiunto_il) IS NOT NULL
...
$artCatTotals = qall($conn, "
  ...
  LEFT JOIN categorie_articoli ca
    ON (a.categoria REGEXP '^[0-9]+$' AND ca.id = CAST(a.categoria AS UNSIGNED))
  WHERE TRIM(COALESCE(a.categoria,'')) <> ''
  GROUP BY categoria ORDER BY n DESC LIMIT 6
");
