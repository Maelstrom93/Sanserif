...
function saveLibro(mysqli $conn, array $dati, array $scelte): bool {
    $stmt = $conn->prepare("
        INSERT INTO libri (titolo, sinossi, immagine, data_pubblicazione, casa_editrice, link, aggiunto_il)
        VALUES (?,?,?,?,?, ?, NOW())
    ");
    $stmt->bind_param(
        'ssssss',
...
function uploadCover(array $file): string {
    ...
    if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true)) {
        error_log("uploadCover: impossibile creare directory $uploadDir");
        return '';
    }
    ...
    error_log("uploadCover: errore spostamento file in $target");
    return '';
}
