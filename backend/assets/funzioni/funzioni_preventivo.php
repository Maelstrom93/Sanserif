 ...
 * CONNESSIONE DB
 * =========================== */
function db_connect_preventivi(
    string $host = 'localhost',
    string $user = 'root',
    string $pass = '',
    string $db   = 'my_sansserifse'
): mysqli {
    $conn = new mysqli($host, $user, $pass, $db);
    if ($conn->connect_error) {
        throw new RuntimeException('Errore connessione DB: ' . $conn->connect_error);
    }
    // Charset consigliato per UTF-8
